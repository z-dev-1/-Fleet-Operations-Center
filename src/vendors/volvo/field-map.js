'use strict';
/**
 * vendors/volvo/field-map.js -- Volvo/ASIST portal field descriptors [V-C]
 * S23-6 (2026-06-28): Selector arrays + value extractors for volvopg.asist.decisiv.net
 *
 * Mirrors PACCAR (S23-6a) with ASIST-specific selector fallbacks:
 *   chassis_number  -- DTNA VIN field on ASIST profile  (VIN synonym)
 *   fleet_number    -- unit number synonym on ASIST profile
 *
 * Each descriptor:
...
const FIELDS = [
  {
    key:       'vin',
    label:     'VIN / Chassis',
    // chassis_number = DTNA VIN field on ASIST profile
    selectors: ['[name=vin]', '[name=chassis_number]', '[name=vehicle_vin]', '[data-field=vin]', '#vin'],
    required:  true,
    getValue:  (unit)         => unit.vin        || unit.vehicleId   || '',
  },
  {
    key:       'unit_number',
    label:     'Unit / Fleet Number',
    // fleet_number = unit_number synonym on ASIST profile
    selectors: ['[name=unit_number]', '[name=fleet_number]', '[name=asset_id]', '[data-field=unit_number]'],
    required:  false,
