...
function _buildEmailReport(userMsg, rows, notesStore, allowedOperators) {
  ...
  if (Array.isArray(allowedOperators) && allowedOperators.length) {
    ...
    if (_allow.length) {
      rows = (rows || []).filter(function(r){ return _allow.indexOf((r.operator||'').toUpperCase()) !== -1; });
    }
  }
...
async function processOrchaAction(userMsg, opts = {}) {
  requireStringMax(userMsg, 'userMsg', MAX_PROMPT_LEN);
    ...
    const richContext = _fleetRelevant ? buildFleetContext(userMsg, {
    ...
    try {
      ...
      for (const a of (parsed.actions||[])) {
        ...
        if (a.type==='SLACK'&&a.recipient&&a.message) {
          ...
          const _scContact = allContacts.find(function(ct){ return (ct.name && ct.name.toLowerCase().includes(_scRL)) || (ct.slackId && ct.slackId.toLowerCase() === _scRL) || (ct.email && ct.email.toLowerCase() === _scRL); });
          const _scOps = (_scContact && Array.isArray(_scContact.operators)) ? _scContact.operators : [];
        ...
        if (a.type==='EMAIL') {
          try {
            ...
            if (!toAddr.includes('@')) {
              ...
              const _emScOps = (_emC && Array.isArray(_emC.operators)) ? _emC.operators : [];
        ...
        if (a.type==='ASK_INTERNAL' && a.question) {
          // Relay to the internal Amazon AI agent (AITeammate) and wait for its
          // guidance. Its answer becomes the reply Z sees. Synchronous by design:
          // Z asked a question and is waiting on the answer.
          try {
            const { askInternal } = require('../orcha/ask-internal');
            logger.info('[ai:orcha-action] ASK_INTERNAL relaying to AITeammate: ' + String(a.question).slice(0, 120));
            const air = await askInternal(a.question);
      ...
      if (_internalAnswer) {
        ...
        chatHistory.push({role:'user', text:userMsg, ts:Date.now()});
        chatHistory.push({role:'ai', text:_internalAnswer, ts:Date.now()});
        store.save('chatHistory', chatHistory);
        return {ok:true,text:_finalInternal,action:'internal',source:'AITeammate'};
      }
