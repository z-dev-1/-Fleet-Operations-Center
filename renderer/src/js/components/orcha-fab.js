...
async function _send() {
  ...
  if(_lcVerb&&_stateM&&_uidsM){var _raw=_stateM[1].toLowerCase(),_lcS=_raw=="available"?"Active":_raw=="unavailable"?"Unavailable":_raw=="ordered"?"Ordered":"End of Life",_rs=(state.slice("fleet").rows)||[],_u=null;for(var _wi=0;_wi<_uidsM.length;_wi++){var _q=_uidsM[_wi].toUpperCase(),_f=_rs.find(function(r){return r.equipmentId&&r.equipmentId.toUpperCase().indexOf(_q)!==-1;});if(_f&&_f.assetUrl){_u=_f;break;}}if(_u&&_u.assetUrl){var _rM=val.match(/\b(?:reason|because|for)\s+(.+)$/i),_lr=_rM?_rM[1].trim():(_lcS=="Active"?"Healthy":"");var _openU=parseInt(_u.openUnplanned,10)||0,_openP=parseInt(_u.openPlanned,10)||0,_hasWO=(_openU+_openP)>0,_override=/\b(repaired|repair complete|complete|fixed|done|anyway|insist|override|i know|proceed|go ahead|yes)\b/i.test(val);if(_lcS=="Active"&&_hasWO&&!_override){inp.value="";inp.style.height="auto";_appendMsg("oc-msg--user",val);_addHistory("user",val);var _woMsg="\u26a0\ufe0f Unit "+_u.equipmentId+" still has an OPEN work order ("+(_openU?_openU+" unplanned":"")+(_openU&&_openP?", ":"")+(_openP?_openP+" planned":"")+(_u.vendor?" \u2014 "+_u.vendor:"")+"). AAP may block the flip. If the repair is complete, say \u201Cflip "+_u.equipmentId+" to active, repaired\u201D to proceed anyway.";_appendMsg("oc-msg--orcha",_woMsg);_addHistory("assistant",_woMsg);return;}inp.value="";inp.style.height="auto";_appendMsg("oc-msg--user",val);_addHistory("user",val);var _lcSt=document.getElementById("orcha-status");if(_lcSt)_lcSt.textContent="\u25CF Setting lifecycle...";try{var _r=await window.aap.setLifecycle(_u.equipmentId,_u.assetUrl,_lcS,_lr);if(_r&&_r.success){var _ok="\u2705 Unit "+_u.equipmentId+" set to "+_lcS+(_lr?" \u2014 "+_lr:"")+".";_appendMsg("oc-msg--orcha",_ok);_addHistory("assistant",_ok);}else{var _fl="\u274c Failed: "+((_r&&_r.message)||"AAP did not confirm.");_appendMsg("oc-msg--orcha",_fl);_addHistory("assistant",_fl);}}catch(_le){_appendMsg("oc-msg--orcha","\u274c Error: "+_le.message);}if(_lcSt)_lcSt.textContent="\u25CF Ready";return;}}
